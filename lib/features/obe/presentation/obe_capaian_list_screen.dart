...
class ObeCapaianListScreen extends ConsumerWidget {
  final String courseId;

  const ObeCapaianListScreen({super.key, required this.courseId});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // Reusing obeEvaluationProvider for class list data as it matches the mockup
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Rata-Rata Capaian', style: AppTextStyles.heading1),
            ...
            evalsAsync.when(
              data: (evals) => Column(
                children: evals.asMap().entries.map((entry) {
                  return _buildClassCard(context, entry.key + 1, entry.value);
                }).toList(),
              ),
  ...
  Widget _buildClassCard(BuildContext context, int index, ObeEvaluationModel eval) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () => context.push('/obe/course/$courseId/capaian/${eval.id}'),
